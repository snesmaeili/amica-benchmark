"""Vendored I/O for the reference Fortran AMICA 1.7 binary (.fdt / .param / outputs).

PROVENANCE: verbatim copy of ``amica_python/benchmark/parity/fortran_io.py`` from the
``ci-tests-improvements`` branch (Fortran-parity track). The comparator lives on
``jax-performance-pass``, which does not carry the parity package, so this small,
numpy-only module is vendored here to keep ``run_fortran.py`` self-contained. The
on-disk format mirrors the EEGLAB Fortran AMICA and is stable, so drift risk is low;
re-sync from the parity branch if that format ever changes.
...
def write_param(path, *, files, outdir, n_channels, n_samples, block_size=None, **overrides):
    ...
    ``files``/``outdir`` are the (here native) paths; ``outdir`` needs a trailing
    slash. ``data_dim`` = n_channels, ``field_dim`` = n_samples. Any AMICA parameter
    can be overridden by keyword (e.g. ``num_mix_comps=3``, ``do_newton=0``).
    """
    path = Path(path)
